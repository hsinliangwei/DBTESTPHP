...
$link = mysqli_connect(
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // 查詢要刪除的數據
        $selectQuery = "SELECT * FROM 教學大綱資料表 WHERE $condition";
        ...
        // 執行刪除數據的 SQL 語句
        $deleteQuery = "DELETE FROM 教學大綱資料表 WHERE $condition";
        ...
        try {
            ...
            if ($affectedRows > 0) {
                ...
                foreach ($deletedData as $data) {
                    $message .= "課程編號：{$data['課程編號']}, 必選修：{$data['必選修']}, 學分數：{$data['學分數']}, 上課時間：{$data['上課時間']}, 課程描述：{$data['課程描述']}<br>";
                }
            } else {
                $message = "<p class='error-message'>未找到符合條件的教學大綱資料</p>";
            }
        } catch (Exception $e) {
            $message = "<p class='error-message'>刪除教學大綱資料失敗！</p>";
            $message .= "<p class='error-message'>" . $e->getMessage() . "</p>";
        }
    

        // 查詢教學大綱資料，按照教學大綱編號的升序排序
        $query = "SELECT 教學大綱編號 FROM 教學大綱資料表 ORDER BY 教學大綱編號 ASC";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // 更新教學大綱編號為當前的編號值
            $update_query = "UPDATE 教學大綱資料表 SET 教學大綱編號 = '$current_id' WHERE 教學大綱編號 = '$id'";
...
<body>
    ...
    <form method="POST" action="">
        <label for="input_id">請輸入要刪除的教學大綱編號：</label>
