...
$link = mysqli_connect(
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($input_id)) {
            $conditions[] = "學歷編號 = '$input_id'";
        }
    
        $condition = implode(" OR ", $conditions);
    
        // 查詢要刪除的數據
        $selectQuery = "SELECT * FROM 學歷資料表 WHERE $condition";
        ...
        // 執行刪除數據的 SQL 語句
        $deleteQuery = "DELETE FROM 學歷資料表 WHERE $condition";
        ...
        try {
            ...
            if ($affectedRows > 0) {
                ...
                foreach ($deletedData as $data) {
                    $message .= "學校名稱：{$data['學校名稱']}, 科系名稱：{$data['科系名稱']}, 學位：{$data['學位']}<br>";
                }
            } else {
                $message = "<p class='error-message'>未找到符合條件的學歷資料</p>";
            }
        } catch (Exception $e) {
            $message = "<p class='error-message'>刪除學歷資料失敗！</p>";
            $message .= "<p class='error-message'>" . $e->getMessage() . "</p>";
        }
    

        // 查詢學歷資料，按照學歷編號的升序排序
        $query = "SELECT 學歷編號 FROM 學歷資料表 ORDER BY 學歷編號 ASC";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // 更新學歷編號為當前的編號值
            $update_query = "UPDATE 學歷資料表 SET 學歷編號 = '$current_id' WHERE 學歷編號 = '$id'";
...
<body>
    ...
    <form method="POST" action="">
        <label for="input_id">請輸入要刪除的學歷編號：</label>
