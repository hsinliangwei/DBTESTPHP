...
$link = mysqli_connect(
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($input_id)) {
            $conditions[] = "經歷編號 = '$input_id'";
        }
    
        $condition = implode(" OR ", $conditions);
    
        // 查詢要刪除的數據
        $selectQuery = "SELECT * FROM 經歷資料表 WHERE $condition";
        ...
        // 執行刪除數據的 SQL 語句
        $deleteQuery = "DELETE FROM 經歷資料表 WHERE $condition";
        ...
        try {
            ...
            if ($affectedRows > 0) {
                ...
                foreach ($deletedData as $data) {
                    $message .= "單位名稱：{$data['單位名稱']}, 職稱：{$data['職稱']}<br>";
                }
            } else {
                $message = "<p class='error-message'>未找到符合條件的經歷資料</p>";
            }
        } catch (Exception $e) {
            $message = "<p class='error-message'>刪除經歷資料失敗！</p>";
            $message .= "<p class='error-message'>" . $e->getMessage() . "</p>";
        }
    

        // 查詢經歷資料，按照經歷編號的升序排序
        $query = "SELECT 經歷編號 FROM 經歷資料表 ORDER BY 經歷編號 ASC";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // 更新經歷編號為當前的編號值
            $update_query = "UPDATE 經歷資料表 SET 經歷編號 = '$current_id' WHERE 經歷編號 = '$id'";
...
<body>
    ...
    <form method="POST" action="">
        <label for="input_id">請輸入要刪除的經歷編號：</label>
