...
$link = mysqli_connect(
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($input_id)) {
            $conditions[] = "信箱編號 = '$input_id'";
        }
    
        if (!empty($input_email)) {
            $conditions[] = "信箱 = '$input_email'";
        }
    
        $condition = implode(" OR ", $conditions);
    
        // 查詢要刪除的數據
        $selectQuery = "SELECT * FROM 信箱資料表 WHERE $condition";
        ...
        // 執行刪除數據的 SQL 語句
        $deleteQuery = "DELETE FROM 信箱資料表 WHERE $condition";
        ...
        try {
            ...
            if ($affectedRows > 0) {
                ...
                foreach ($deletedData as $data) {
                    $message .= "信箱：{$data['信箱']}<br>";
                }
            } else {
                $message = "<p class='error-message'>未找到符合條件的信箱資料</p>";
            }
        } catch (Exception $e) {
            $message = "<p class='error-message'>刪除信箱失敗！</p>";
            $message .= "<p class='error-message'>" . $e->getMessage() . "</p>";
        }
    

        // 查詢信箱資料，按照信箱編號的升序排序
        $query = "SELECT 信箱編號 FROM 信箱資料表 ORDER BY 信箱編號 ASC";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // 更新信箱編號為當前的編號值
            $update_query = "UPDATE 信箱資料表 SET 信箱編號 = '$current_id' WHERE 信箱編號 = '$id'";
...
<body>
    ...
    <form method="POST" action="">
        <label for="input_id">請輸入要刪除的信箱編號：</label>
        <input type="text" name="input_id">
        <span>或</span>
        <label for="input_email">信箱：</label>
